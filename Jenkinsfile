...
//         }
        stage('Test') {
            steps {
                sh script: "docker run --name k6-docker-${env.SERVICE_ID} -e RAMP_UP=${RAMP_UP} -e HOLD_DURATION=${HOLD_DURATION} -e TEAR_DOWN=${TEAR_DOWN} -e TARGET_LOAD=${TARGET_LOAD} -i ${env.k6_DOCKER_IMAGE} --quiet run petStore/main.js", label: "Run perf test"
            }
            post {
                always {
